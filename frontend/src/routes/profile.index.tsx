...
import { Pencil, ChefHat, Heart, BookOpen, Award, Share2, Sparkles } from "lucide-react";
...
function Profile() {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: `${profile.name}'s Profile — Cultivate`,
        text: `Check out ${profile.name}'s profile on Cultivate!`,
        url: profileUrl,
      })
